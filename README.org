...
** =doxygen=
   ...
   the documentation is [[file:doxygen/html/index.html][=html/index.html=]]. A graphical representation
   of the class hierarchy is available under [[file:doxygen/html/inherits.html][=html/inherits.html=]].
...
**** =controllers=:
     ...
     [[#struct-ode-conf]] below).

**** =managers=:
     Classes that *handle communication with the [[#struct-ode-app-neo4play][database access
     layer]]*.

     Each [[#struct-ode-app-models][model class]] has a static =nodes= field or a static
     ...
     model data. When handling user requests, [[#struct-ode-app-controllers][controllers]] call these
...
*** =conf=
    ...
    <URL>= and [[#struct-ode-app-controllers][controller methods]]:
...
** =training-materials=
   ...
   1. a web server (such as Apache) has to serve the file
      [[file:training-materials/questionaire/persist.php][=persist.php=]] at [[http://localhost/persist.php]]
...
*** Data
    ...
    can create a =.cql= script (or simply extend [[file:initial-data.cql][=initial-data.sql=]])
    and load it as described in Section [[#setup-initial]] above.
...
**** Values
     ...
     [[file:initial-data.cql][=initial-data.sql=]] already contains an appropriate Cypher query
     for adding this node. Just make sure you load it as described in
     Section [[#setup-initial]] above /before/ creating any features.
